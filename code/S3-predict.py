...
if __name__ == '__main__':
    ...
    length = [200, 1000, 2000, 4000, 7000]
    ...
    for nlp, each in zip(name_nlp, nlps):
        ...
        x_train_total, y_train_total = x[train], y[train]

        total_len = x_train_total.shape[0]

        detail = []
        for i in length:
            print "NLP : "+str(nlp)+ " in cycle " + str(i)+ "--------"
            ind = range(i)
            x_train, y_train = x_train_total[ind], y_train_total[ind]
            rmse = evaluate_nlp(nlp, x_train, x_test, y_train, y_test)
            detail.append(rmse)
        scipy.io.savemat('%s.mat' %nlp, mdict={'detail': detail})
